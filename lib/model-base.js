...
var urlGenerator = function urlGenerator(url) {
    ...
    return [urlTokens, _pathToRegexp2.default.tokensToFunction(urlTokens)];
};
...
var ModelBase = function () {
    ...
    _createClass(ModelBase, [{
        ...
        key: '$request',
        value: function $request(urlTokens, options) {
            ...
            // fix params
            _lodash2.default.forEach(urlTokens, function (token) {
                if (token && token.name) {
        ...
        value: function extend(baseUrl, actions, staticProps, options) {
            ...
            _lodash2.default.forEach(actions, function (actionInfo, name) {
                var _urlGenerator = urlGenerator(Model.baseUrl + (actionInfo.url || ''));

                var _urlGenerator2 = _slicedToArray(_urlGenerator, 2);

                actionInfo.tokens = _urlGenerator2[0];
                actionInfo.url = _urlGenerator2[1];
        ...
        value: function addAction(name, actionInfo) {
            ...
            this[name] = function (params) {
                ...
                result.$promise = model.$request(actionInfo.tokens, options).then(function (response) {
